...
When looking at the distrubtion of quality in the red wine data we see a resonable normal distrubtion, there are very few total observations with a quality rating of 3,4 and 8. This may be a factor when looking to predict quality as the datset imbalance may skew results. The white wine datset has a more normal distrbution with a more gradual step down from the peak of the histogram. The number of observations with qaulity of 3 and 9 are extremly low, while 4 and 8 have a relatively low number of observations.
```{r echo=FALSE}
...
When assesing the distributiono of the predictor variables with the use of histograms, there are some similarites across features. In both datasets Density and Ph have a normal distribution, while chlorides, residual sugar and alchol have a non - normal distrubtion and fixed acidity and volatile acidity are seent to be reasonably normal. When comparing the features that differ across the datasets we can see citric acid, free sulfur dioxide, total sulfur dioxide, sulphates appear as a non normal distribution in the red wine data but appear resoanble normal in the white wine data. This leads me to conclude that on the whole the white wine data set has a more normal distrubtion compared to the red wine data. When looking at possible outliers the features that appear to have outliers in both datasets are residual sugar and chlorides, while only in the red wine data does suplahe appera to have obvious outliers. 
...
for (i in seq_along(numeric_vars)) {
  print(gg_histogram(red_wine, numeric_vars[i]))
}

## for loop to create all the boxplots
for (i in seq_along(numeric_vars)) {
  print(gg_histogram(white_wine, numeric_vars[i]))
}
...
## for loop to create all the boxplots
for (i in seq_along(numeric_vars)) {
  print(gg_scatterplot(red_wine, numeric_vars[i]))
}

## for loop to create all the boxplots
for (i in seq_along(numeric_vars)) {
  print(gg_scatterplot(white_wine, numeric_vars[i]))
}
...
## for loop to create all the boxplots
for (i in seq_along(numeric_vars)) {
  print(gg_boxplot(red_wine, numeric_vars[i]))
}
...
**Muliple Linear Regression Metrics**
```{r}
...
**Decsion Tree Metrics**
```{r}
rbind(red_tree.metrics, red_tree.tuned.metrics, white_tree.metrics, white_tree.tuned.metrics)
```
## Random forest Model
The random forest model found the best model fit using 6 features at each split for the red wine dataset and 3 features at each split for the white wine. As more features were introduced into the model R-sqaured values decreased and RMSE increased. The random forest model returned the lowest RMSE and highest R-Sqaure values for both the red and white wine data. The model had a slightly better fit on the white wine data with a RMSE and R-sqaure of 0.60 and 0.53 against the red wine values that were RMSE = 0.63 and R-sqaure = 0.43. The random forest model may have performed better on the data due to its random nature being able to better understand the overlap between feature behavour and reposnse outcome. As we saw with the scatter plots some features had very little impact on quality while some features clearly had a stronger relationship. using its many iterations the random forest was able to trial many of the less significant features and understand how those values impact the prediction. This is a significant improvement on the single decison tree as the single tree struggled to caputre the overall variabilty of the data while the iterations of the random forest are better suited at encapsualting that variability.
...
**Random Forest Plots-White Wine**
```{r}
plot(white.rf_mdl)
...
```
**Random Forest Classification Tuning Plot - Red Wine**

```{r}
plot(red_rf_mdl.C)
```

**Random Forest Classification Confusion Matrix - Red Wine**
```{r}
...
**Random Forest Classification Tuning Plot - White Wine**
```{r}
plot(white_rf_mdl.C)
```

**Random Forest Classification Confusion Matrix - White Wine**
```{r}
...
**Boosted Confusion Matrix - Red Wine**
```{r}
...
**Boosted Confusion Matrix - White Wine**
```{r}
...
## Conclusion
To understand what would be the most approariate alogorthim to apply to these datasets, whether that be regression or classfication. It is important to have a solid understanding of what you would like to achive with these predictions. If you need your prediction to tell you the exact predicted quality of a new opbservation then clearly the classfication method is more appropriate. Instead if you are looking to gain an understaindg of roughly where a new observation will fall in relation to quality then maybe the regression models are more approrpitae. If I was working for a restaurant and was looking to make wine purchases baseds on these features and wanted confidence the wine would be saleable I would use the classfication method  and seek to choose wines that fall in the central qualities. I would prefer the classfication method as I could more easily evalute wine price points against each other as I would have more confidence it is a true 5 or 6 and thus should be priced accordingly. While the regression model should get me 0.6 away from the quality the relatively low Rsqaure values woudl not give me confidence in the prediction and it could be far lower or higher. If I was making a personal purchase and I wanted to find the highest quality wine and was happy to risk it being a lower quality I would use the regression method as the prediction is not as biased by the limited informtion from the data. As the classfication method raraely predicted wines to be on the high end of the scale. If I wanted to make my prediction with confidence I would not use the regression method due to the consistently low Rsqaure values.

```{r}
GGally::ggpairs(red_wine)
```
